...
module.exports = function(tollan) {
			...
			tollan.postAction('blog/addTag', {slug: slugify(this.state.name), name: this.state.name})
				.then(response => {
					if (response.statusCode === 200) {
						...
						tagStore.reload(); // Reload the entire tag store, easier than updating it
	...
	var TagRow = require('./TagRow')(tollan);
	...
	return React.createClass({
		...
		onStoreChange: function() {
			this.setState({tags: tagStore.getAll(), loaded: tagStore.loaded});
		},
		render: function() {
			var tags = this.state.tags.map(tag => {
				return <TagRow key={tag.slug} tag={tag} onTagDeleted={this.onTagDeleted} onTagDeleteFail={this.onTagDeleteFail} onTagSave={this.onTagSave} onTagSaveFail={this.onTagSaveFail} />;
			});
			var addTag;
			...
			var panelContent;
			if (!tagStore.loaded || !this.state.loaded) {
